...
export const DesignerLayout = ({ children }: Props) => {
  ...
  const getCurrentTitle = () => {
    for (const group of DESIGNER_NAV) {
      const match = group.items.find((item) => item.path === location.pathname);
      if (match) return match.label === "Dashboard" ? "Designer Studio" : match.label;
    }
    return "Designer Studio";
  };
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-white border-b border-outline-variant h-14 flex items-center px-4 md:px-6 shrink-0">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <span className="text-xs text-emerald font-semibold">Profile Verified</span>
          </div>
        </header>
